...
class calendar {
	...
	function init()
	{
		...
		switch($this->company){			
				...
				break;
				/*case 'MarionOutdoor':
				$this->xmlfile = array(0 => 'mar.xml', 1 => 'out.xml');
				break;	*/					
			}
						    ...
						    foreach($mo as $dayOrdinal => $day)
						    {
								...
								if( count($day) > 0 ){ //true = yes, there's content to update / add
									...
									foreach($day as $jNbr => $job){										
											   ...
											   $jobNode = $this->doc->createElementNS( 'calendar', 'job', $job );
					    ...
					    for( $t = 0; $dayNode = $searchNode->getElementsByTagName('day')->item($t); $t++ )
					    {
						    ...
						    else  //else1
						    {
							   ...
							   //echo '  Day node date attrib is: ' .$dayNode->getAttribute("date") .' for month '.$searchMonth->getAttribute('ordinal');
								    ...
								    //~~li class="lineEntrylineEntry unassigned"
							    ...
							    if( $this->activeOrdinalCell == $dayNode->getAttribute("ordinal") && (integer)$this->activeMonthOrdinal == (integer)$searchMonth->getAttribute('ordinal') ) //if2
							    {

...
class saveToCalendar  {
						  ...
						  for( $iYr = 0; $y = $this->xmlCompanyNode->getElementsByTagName('year')->item($iYr); $iYr++ ){
							  ...
							  if(isset($this->xmlMonthNode)){
								...
								/*  //if no matching month node found. create it in xml.
								  ...
								  // got our (day), let's find job to write the data!
								  ...
								  if(isset($this->xmlDayNode)){
										...
										echo $ptrn;
										//any job nodes inside this day node?
										if( $this->xmlDayNode->hasChildNodes() ){
											echo "day has child nodes";
											for($iJ = 0; $jNode = $this->xmlDayNode->getElementsByTagName('job')->item($iJ); $iJ++){

											 if( preg_match("'".$ptrn."'", $jNode->getAttribute('number')) !== false )
											 {	
												 $this->xmlJobNode = $jNode;
												echo "Skipping: This Entry Already Exists in the Calendar: Job No ".$csvRow['job']." <br/>";			echo "<br>ptrn is ".$ptrn."<br>";			 
												//unset($this->CSVrows[$rowNmbr]);
												$this->duplicateEntry = true;	
											 }
											}
										} else { //date has no job nodes... 
												echo "this date does not have any job nodes";
												 //return array of duplicates found that don't have unassigned;
												 //and delete any unassigned dups found in the process:
												 $result = $this->exhaustiveSearch($csvRow, $ptrn);
												 ...
												 if( count($result) > 0  || !empty($result) ){
													 ...
													 echo "Array count(result) is 0; duplicate= false";
													 $this->duplicateEntry = false;
												 }
											 }
										 ...
										 {	
												...
												$this->CSVrows[$rowNmbr]['job'].'" #$#' . $csvRow['job'] . '*^*span#$# ' . htmlspecialchars($csvRow['custName']) .'*^*li#$# ';

												$newJobNode = $this->doc->createElement( 'job', $appendToNode );
	...
	*/
	...
	private function exhaustiveSearch($csvRow,$ptrn){

...
function AJAXsynchronous($method){
	...
	function editEmployee(btn){
		...
		if( $('#directory').find('.editable') ){
			...
			var b = $('.editable').children('#updateCancel');
			cancel(b);	
		...
		if($(btn).text() === 'Save'){
			...
			var editForm = '<form method="post" id="saveUpdates">'+
						...
						'</form>&nbsp; <button id="updateRow" class="smBtn inline four" empid="'+ empid + '" onclick="test(this)">Save</button>'+ '&nbsp; <button id="updateCancel" class="smBtn inline six"'+
	...
	*/
	...
	function cancel(btn){
		...
		var $origRow = $(btn).parent('.editable');
		$($origRow).removeClass('editable ghosted');
		$($origRow).children('#editEmployee').removeClass('hide');		
		$($origRow).children('form#saveUpdates').remove();
		$($origRow).children('#updateRow').remove();		
		$($origRow).children('#updateCancel').remove();
		//$($origRow).children('br').remove();
